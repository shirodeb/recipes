...
export PACKAGE="com.github.haveagitgat.hbbatchbeast"
export NAME="HBBatchBeast"
export VERSION="2.1.5"
export ARCH=$(utils.misc.get_current_arch)
export URL="https://github.com/HaveAGitGat/HBBatchBeast/releases/download/2.1.5/hbbatchbeast-Linux-v2.1.5.deb"
# autostart,notification,trayicon,clipboard,account,bluetooth,camera,audio_record,installed_apps
export REQUIRED_PERMISSIONS=""

export DESC1="A free GUI application for HandBrake and FFmpeg/FFprobe with an emphasis on batch conversion (including recursive folder scans and folder watching) -Windows, macOS, Linux & Docker"
export DESC2=""
...
export SECTION="misc"
export PROVIDE=""
export HOMEPAGE="https://github.com/HaveAGitGat/HBBatchBeast"
export AUTHOR="HaveAGitGat"

function build() {
    local natives=($(find ${SRC_DIR}/ -name "natives_blob.bin"))
    local root_dir=$(dirname ${natives[0]})
    pushd $root_dir
    cp -r * ${APP_DIR}/files/
    local chrome_sandboxes=($(find ${APP_DIR}/files -name "chrome[-_]sandbox"))
    local natives=($(find ${APP_DIR}/files -name "natives_blob.bin"))
    local root_dir=$(dirname ${natives[0]})
